...
db_name = 'lvi3plp-stg'
table_name = 'ORION_EDW.JSON_DOCUMENT_STG'
...
def main2():
    file_type = "DOCUMENT"
    date_to_process = datetime(2023, 10, 26).date()  # Change this to the desired date
...
def refreshHeaderMV(db_credentials):
    connHdr = redshift_connector.connect(
    ...
    # execute SP to refresh Materialized views needed for Document IC process
    logger.info("Refreshing Materialized Views")
    cursorHdr = connHdr.cursor()
    cursorHdr.execute("call orion_edw.proc_document_refresh_stg_mv_hdr();")
    connHdr.commit()
    cursorHdr.close()
    connHdr.close()


def refreshDetailMV(db_credentials):
    connDtl = redshift_connector.connect(
        host=db_credentials['host'],
        port=5439,
        database=db_name,
        user=db_credentials['username'],
    ...
    # execute SP to refresh Materialized views needed for Document IC process
    logger.info("Refreshing Materialized Views")
    cursorDtl = connDtl.cursor()
    cursorDtl.execute("call orion_edw.proc_document_refresh_stg_mv_dtl();")
    connDtl.commit()
    cursorDtl.close()
    connDtl.close()
...
def main():
    try:
        ##################################################################################
        # dt = datetime(2023, 10, 26).date()  # Change this to the desired date
        # datesToCheck=[dt] #This is needed to check if there are pending files from previous date that needs to be loaded
        ...
        for date_to_process in datesToCheck:
            ...
            if iPendingFilesToProcess > 0:
                ...
                cursor = conn.cursor()
                cursor.execute(f"""
                        COPY {table_name}(facility_id,source_system_code,filename,filename_chunk,process_status,parsing_date,xml_data) 
                        FROM '{s3_path}' 
                        IAM_ROLE '{iam_role}' 
                        FORMAT JSON 'auto'
                        GZIP;
        ...
        # execute SP to refresh Materialized views needed for Document IC process
        logger.info("Refreshing Materialized Views - Setting the flag in Y")
        cursor = conn.cursor()
        # cursor.execute("UPDATE ""orion_edw"".""amc_parameter"" SET value_start='Y' WHERE name='DOC_MV_REFRESH_IN_PROGRESS';")
        cursor.execute(
            "update ORION_EDW.AMC_PARAMETER set value_start='Y' WHERE name in ('DOC_MV_REFRESH_IN_PROGRESS_HDR','DOC_MV_REFRESH_IN_PROGRESS_DTL');")
        conn.commit()

        # execute SP to refresh Materialized views needed for Document IC process
        logger.info("Refreshing Materialized Views")
        thread_one = threading.Thread(target=refreshDetailMV, args=(db_credentials,))
        thread_two = threading.Thread(target=refreshHeaderMV, args=(db_credentials,))
        thread_one.start()
        thread_two.start()
        thread_one.join()
        thread_two.join()
        ...
        # cursor = conn.cursor()
        # cursor.execute("call orion_edw.proc_document_refresh_stg_mv();")
        # conn.commit()
