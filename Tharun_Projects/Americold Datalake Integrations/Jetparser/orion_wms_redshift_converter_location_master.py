...
folder = "LOCATION_MASTER"

update = True
# Replace these values with your own
db_name = 'lvi3plp-stg'
table_name = 'ORION_EDW.JSON_LOCATION_MASTER_STG'  # ACA HAY QUE CAMBIAR A LA TABLA QUE VAYAMOS A USAR!!
...
def delete_older_entries(cursor, conn):
    try:
        # Delete older entries
        cursor.execute("""
            DELETE 
            FROM 
                ORION_EDW.json_location_master_stg
            WHERE 
                (SOURCE_SYSTEM_CODE, FACILITY_ID) IN (
                    SELECT 
                        SOURCE_SYSTEM_CODE, 
                        FACILITY_ID
                    FROM 
                        (
                            SELECT 
                                SOURCE_SYSTEM_CODE, 
                                FACILITY_ID, 
                                COUNT(DISTINCT PARSING_DATE) as n_dates, 
                                MAX(parsing_date) as max_date
                            FROM 
                                ORION_EDW.json_location_master_stg
                            GROUP BY 
                                SOURCE_SYSTEM_CODE, 
                    ...
                    FROM 
                        (
                            ...
                            GROUP BY 
                                SOURCE_SYSTEM_CODE, 
                                FACILITY_ID
                        ) as sub_max_date_query
                    WHERE 
                        sub_max_date_query.SOURCE_SYSTEM_CODE = json_location_master_stg.SOURCE_SYSTEM_CODE
                        AND 
                        sub_max_date_query.FACILITY_ID = json_location_master_stg.FACILITY_ID
                );
        """)
        ...
        logger.error(f"Error erasing old values for stg table: {e}")


def delete_obsolete_location_data(cursor, conn):
    try:
        query = """
        DELETE FROM globali3pl.location_master
        WHERE source_system_code || '-' || facility_id IN
        (
        SELECT vw.source_system_code || '-' || vw.facility_id AS site
        FROM (
            SELECT source_system_code, facility_id, MAX(create_date) AS max_dt
            FROM ORION_EDW.location_master_json_vw
            GROUP BY source_system_code, facility_id
        ) vw
        JOIN (
            SELECT source_system_code, facility_id, MAX(wms_create_dt) AS max_dt
            FROM globali3pl.location_master
            GROUP BY source_system_code, facility_id
        ) lm 
        ON vw.facility_id = lm.facility_id AND vw.source_system_code = lm.source_system_code
        AND vw.max_dt > lm.max_dt 
        );
        """
        cursor.execute(query)
        logger.info("Obsolete location master data deleted successfully.")
        conn.commit()
    except Exception as e:
        logger.error(f"Error while deleting obsolete location master data: {e}")


def insert_new_location_data(cursor, conn):
    try:
        query = """
        INSERT INTO globali3pl.location_master
        (facility_id, source_system_code, location_id, temperature_uom, linear_uom, weight_uom
        , building_id, room_id, temperature, aisle_id, bin_id, level_id
        , eng_capacity, total_pp, used_pp, available_pp, locked_pp, rack_height
        , max_prod, max_lot, fragm_pct, rack_max_weight, wms_create_dt, load_dt)
        SELECT facility_id, source_system_code, location_id, temperature_uom, linear_uom, weight_uom
                , building_id, room_id, temperature, aisle_id, bin_id, level_id
                , eng_capacity, total_pp, used_pp, available_pp, locked_pp, rack_height
                , max_prod, max_lot, fragm_pct, rack_max_weight
                , create_date as wms_create_dt
                , current_timestamp as load_dt 
        FROM ORION_EDW.location_master_json_vw
        WHERE source_system_code || '-' || facility_id  || '-' || to_char(create_date,'YYYYMMDD') IN (
        SELECT vw.source_system_code || '-' || vw.facility_id  || '-' || to_char(vw.max_dt,'YYYYMMDD') AS new_site
        FROM (
            SELECT source_system_code, facility_id, MAX(create_date) AS max_dt
            FROM ORION_EDW.location_master_json_vw
            GROUP BY source_system_code, facility_id
        ...
        ON vw.facility_id = lm.facility_id AND vw.source_system_code = lm.source_system_code
        WHERE lm.facility_id IS NULL
        );
        """
        cursor.execute(query)
        logger.info("New location master data inserted successfully.")
        conn.commit()
    except Exception as e:
        logger.error(f"Error while inserting new location master data: {e}")
    ...
    for folder_name in ["in_progress", "loaded"]:
        ...
        try:
            ...
            if e.response["Error"]["Code"] == "404":
                # key doesn't exist and needs to be created
...
def isValidFile(sObjKey, sSourcePath):
    if (sObjKey.endswith('.json.gz')):  # Check if it's a file and as well, if it's in the right DAY path
...
def main2():
    file_type = "LOCATION_MASTER"
    date_to_process = datetime(2023, 10, 31).date()  # Change this to the desired date
    source_folder_key = f"{file_type}/{date_to_process.strftime('%Y/%m/%d')}/"
    destination_folder = f"{file_type}/{date_to_process.strftime('%Y/%m/%d')}/loaded/"
...
def main():
    try:
        ...
        # dt = datetime(2023, 10, 29).date()
        datesToCheck = [dt - timedelta(days=1),
                        dt]  # This is needed to check if there are pending files from the previous date that needs to be loaded
        ...
        for date_to_process in datesToCheck:
            ...
            if iPendingFilesToProcess > 0:
                ...
                # Delete old files from staging table and keep only the most updated version
                delete_older_entries(cursor, conn)

                if update == True:
                    # Delete obsolete data in master location.
                    delete_obsolete_location_data(cursor, conn)

                    # Insert new location data
                    insert_new_location_data(cursor, conn)

                conn.close()

    except Exception as ex:
        logger.error(f"Error loading data for {date_to_process}: {ex}")
